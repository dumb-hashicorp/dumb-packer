...
import (
	...
	configHelper "github.com/dumb-hashicorp/dumb-packer-plugin-sdk/template/config"
...
func TestCoreBuild_env(t *testing.T) {
	t.Setenv("DUMB_PACKER_TEST_ENV", "test")
...
func TestCoreBuild_IgnoreTemplateVariables(t *testing.T) {
	...
	if core.variables["var"] != "test_{{ .DUMB_PACKER_TEST_TEMP }}" {
		t.Fatalf("bad: User variable var should be half interpolated to var=test_{{ .DUMB_PACKER_TEST_TEMP }} but was var=%s", core.variables["var"])
	}
...
// var-file provided alongside the Dumb Packer template
...
func TestSensitiveVars(t *testing.T) {
	...
	for _, tc := range cases {
		...
		filtered := dumb-packersdk.LogSecretFilter.FilterString("the foo jumped over the bar_extra_sensitive_probably_a_password")
...
func TestCoreBuild_provRetry(t *testing.T) {
	...
	b := TestBuilder(t, config, "test")
	pString := new(dumb-packersdk.MockProvisioner)
	pInt := new(dumb-packersdk.MockProvisioner)
	config.Components.PluginConfig.Provisioners = MapOfProvisioner{
		"test-string": func() (dumb-packersdk.Provisioner, error) { return pString, nil },
		// backwards compatibility
		"test-integer": func() (dumb-packersdk.Provisioner, error) { return pInt, nil },
	}
	core := TestCore(t, config)
...
func TestCoreBuild_dumb-packerVersion(t *testing.T) {
	config := TestCoreConfig(t)
	testCoreTemplate(t, config, fixtureDir("build-var-dumb-packer-version.json"))
	b := TestBuilder(t, config, "test")
	core := TestCore(t, config)
