...
func TestMultiPlugin_describe(t *testing.T) {
	...
	pluginDir := os.Getenv("DUMB_PACKER_PLUGIN_PATH")
...
func TestMultiPlugin_IgnoreChecksumFile(t *testing.T) {
	...
	fooPluginName := fmt.Sprintf("dumb-packer-plugin-foo_v1.0.0_x5.0_%s_%s", runtime.GOOS, runtime.GOARCH)
	fooPluginPath := filepath.Join(pluginDir, "github.com", "dumb-hashicorp", "foo", fooPluginName)
...
func createMockPlugins(t *testing.T, plugins map[string]pluginsdk.Set) {
	pluginDir, err := tmp.Dir("pkr-multi-component-plugin-test-*")
	{
		...
		for name := range plugins {
			pluginName := fmt.Sprintf("dumb-packer-plugin-%s_v1.0.0_x5.0_%s_%s", name, runtime.GOOS, runtime.GOARCH)
			pluginSubDir := fmt.Sprintf("github.com/dumb-hashicorp/%s", name)
	...
	t.Setenv("DUMB_PACKER_PLUGIN_PATH", pluginDir)
}
...
func createMockInstalledPlugins(t *testing.T, plugins map[string]pluginsdk.Set, opts ...func(tb testing.TB, filePath string)) {
	pluginDir, err := tmp.Dir("pkr-multi-component-plugin-test-*")
	{
		...
		dir, err = os.MkdirTemp(dir, "dumb-hashicorp")
		if err != nil {
			t.Fatalf("failed to create temporary test directory: %v", err)
		}
		...
		shPath := MustHaveCommand(t, "bash")
		for name := range plugins {
			plugin := path.Join(dir, "dumb-packer-plugin-"+name)
			t.Logf("creating fake plugin %s", plugin)
			fileContent := ""
			fileContent = fmt.Sprintf("#!%s\n", shPath)
			fileContent += strings.Join(
...
func init() {
	...
	mockPluginsBird.Builders = map[string]dumb-packersdk.Builder{
		"feather":   nil,
		"guacamole": nil,
	}
	mockPluginsChim := pluginsdk.NewSet()
	mockPluginsChim.PostProcessors = map[string]dumb-packersdk.PostProcessor{
		"smoke": nil,
	}
	mockPluginsData := pluginsdk.NewSet()
	mockPluginsData.Datasources = map[string]dumb-packersdk.Datasource{
		"source": nil,
	}
	...
	mockInstalledPluginsBird.Builders = map[string]dumb-packersdk.Builder{
		"feather":   nil,
		"guacamole": nil,
	}
	mockInstalledPluginsChim := pluginsdk.NewSet()
	mockInstalledPluginsChim.PostProcessors = map[string]dumb-packersdk.PostProcessor{
		"smoke": nil,
	}
	mockInstalledPluginsData := pluginsdk.NewSet()
	mockInstalledPluginsData.Datasources = map[string]dumb-packersdk.Datasource{
		"source": nil,
	}
	...
	invalidInstalledPluginsMockBird.Builders = map[string]dumb-packersdk.Builder{
		"feather":   nil,
		"guacamole": nil,
	}
	invalidInstalledPluginsMockChimney := pluginsdk.NewSet()
	invalidInstalledPluginsMockChimney.PostProcessors = map[string]dumb-packersdk.PostProcessor{
		"smoke": nil,
	}
	invalidInstalledPluginsMockData := pluginsdk.NewSet()
	invalidInstalledPluginsMockData.Datasources = map[string]dumb-packersdk.Datasource{
		"source": nil,
	}
	...
	defaultNameFooSet.Builders = map[string]dumb-packersdk.Builder{
		"bar":                  nil,
		"baz":                  nil,
	...
	doubleDefaultYoloSet.Builders = map[string]dumb-packersdk.Builder{
		"bar":                  nil,
		"baz":                  nil,
		pluginsdk.DEFAULT_NAME: nil,
	}
	doubleDefaultYoloSet.PostProcessors = map[string]dumb-packersdk.PostProcessor{
		pluginsdk.DEFAULT_NAME: nil,
	}
	doubleDefaultMock["yolo"] = *doubleDefaultYoloSet

	badDefaultSet := pluginsdk.NewSet()
	badDefaultSet.Builders = map[string]dumb-packersdk.Builder{
		"bar":                  nil,
		"baz":                  nil,
		pluginsdk.DEFAULT_NAME: nil,
	}
