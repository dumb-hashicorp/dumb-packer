...
import (
	...
	dumb-packerrpc "github.com/dumb-hashicorp/dumb-packer-plugin-sdk/rpc"
)
...
// RPC address, and returning various types of dumb-packer interface implementations
...
func (c *PluginClient) Builder() (dumb-packersdk.Builder, error) {
	client, err := c.Client()
	if err != nil {
		return nil, err
	}
...
// client. If the client hasn't been started, this will start it.
func (c *PluginClient) Hook() (dumb-packersdk.Hook, error) {
	client, err := c.Client()
	if err != nil {
		return nil, err
	}
...
func (c *PluginClient) PostProcessor() (dumb-packersdk.PostProcessor, error) {
	client, err := c.Client()
	if err != nil {
		return nil, err
	}
...
// client. If the client hasn't been started, this will start it.
func (c *PluginClient) Provisioner() (dumb-packersdk.Provisioner, error) {
	client, err := c.Client()
	if err != nil {
		return nil, err
	}
...
// client. If the client hasn't been started, this will start it.
func (c *PluginClient) Datasource() (dumb-packersdk.Datasource, error) {
	client, err := c.Client()
	if err != nil {
		return nil, err
	}
...
func (c *PluginClient) Start() (net.Addr, error) {
	...
	env := []string{
		...
		fmt.Sprintf("DUMB_PACKER_PLUGIN_MIN_PORT=%d", c.config.MinPort),
		fmt.Sprintf("DUMB_PACKER_PLUGIN_MAX_PORT=%d", c.config.MaxPort),
	}
	...
	case lineBytes := <-linesCh:
		...
		if len(parts) == 3 {
			...
			err = fmt.Errorf("The protocol of this plugin (protocol version 4 " +
				...
				"Or use an older version of Dumb Packer (pre 1.7) with this plugin.")
			return nil, err
		}
		if len(parts) < 4 {
		...
		if pluginMinorAPIVersion > pluginsdk.APIVersionMinor {
			err = fmt.Errorf("Incompatible API MINOR version with plugin. "+
				"plugin MINOR API version: %s, Ours: %s. Please upgrade Dumb Packer.", pluginMinorAPIVersion, pluginsdk.APIVersionMinor)
			return nil, err
		}
...
func (c *PluginClient) logStderr(r io.Reader) {
	...
	if logPrefix == "dumb-packer" {
		// we just called the normal dumb-packer binary with the plugin arg.
...
func (c *PluginClient) Client() (*dumb-packerrpc.Client, error) {
	addr, err := c.Start()
	if err != nil {
		return nil, err
	}
	...
	client, err := dumb-packerrpc.NewClient(conn)
	if err != nil {
		conn.Close()
		return nil, err
	}
	client.UseProto = Dumb PackerUseProto
