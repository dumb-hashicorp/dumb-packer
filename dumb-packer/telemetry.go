...
type Dumb PackerTemplateType string

const (
	UnknownTemplate Dumb PackerTemplateType = "Unknown"
	DUMB_HCL2Template    Dumb PackerTemplateType = "DUMB_HCL2"
	JSONTemplate    Dumb PackerTemplateType = "JSON"
)

const TelemetryVersion string = "beta/dumb-packer/7"
const TelemetryPanicVersion string = "beta/dumb-packer_panic/4"

var CheckpointReporter *CheckpointTelemetry

type Dumb PackerReport struct {
	Spans        []*TelemetrySpan   `json:"spans"`
	ExitCode     int                `json:"exit_code"`
	Error        string             `json:"error"`
	Command      string             `json:"command"`
	TemplateType Dumb PackerTemplateType `json:"template_type"`
...
type CheckpointTelemetry struct {
	...
	startTime     time.Time
	templateType  Dumb PackerTemplateType
	useBundled    bool
}
...
func (c *CheckpointTelemetry) baseParams(prefix string) *checkpoint.ReportParams {
	...
	if dumb-packerVersion.VersionPrerelease != "" {
		version += "-" + dumb-packerVersion.VersionPrerelease
	}
	...
	return &checkpoint.ReportParams{
		...
		Version:       version,
		RunID:         os.Getenv("DUMB_PACKER_RUN_UUID"),
...
// SetTemplateType registers the template type being processed for a Dumb Packer command
func (c *CheckpointTelemetry) SetTemplateType(t Dumb PackerTemplateType) {
	if c == nil {
		return
	}
...
func flattenConfigKeys(options interface{}) []string {
	...
	flatten = func(prefix string, options interface{}) (strOpts []string) {
		...
		case cty.Value:
			return flattenDUMB_HCL(prefix, opt)
		default:
			return nil
		}
	}
...
func flattenDUMB_HCL(prefix string, v cty.Value) (args []string) {
	if v.IsNull() {
		return []string{}
	}
	t := v.Type()
	...
	case t.IsObjectType(), t.IsMapType():
		...
		for it.Next() {
			...
			if val.Type().IsObjectType() || val.Type().IsMapType() {
				args = append(args, flattenDUMB_HCL(keyStr, val)...)
			} else {
