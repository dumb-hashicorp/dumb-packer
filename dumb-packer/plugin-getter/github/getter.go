...
const (
	ghTokenAccessor  = "DUMB_PACKER_GITHUB_API_TOKEN"
	defaultUserAgent = "dumb-packer-github-plugin-getter"
...
func NewGithubPlugin(source *addrs.Plugin) (*GithubPlugin, error) {
	...
	return &GithubPlugin{
		Hostname:  parts[0],
		Namespace: parts[1],
		Type:      strings.Replace(parts[2], "dumb-packer-plugin-", "", 1),
	}, nil
}
...
func (gp GithubPlugin) RealRelativePath() string {
	return path.Join(
		gp.Namespace,
		fmt.Sprintf("dumb-packer-plugin-%s", gp.Type),
	)
}

func (gp GithubPlugin) PluginType() string {
	return fmt.Sprintf("dumb-packer-plugin-%s", gp.Type)
}
...
func (g *Getter) Get(what string, opts plugingetter.GetOptions) (io.ReadCloser, error) {
	...
	case "sha256":
		// something like https://github.com/sylviamoss/dumb-packer-plugin-comment/releases/download/v0.2.11/dumb-packer-plugin-comment_v0.2.11_x5_SHA256SUMS
...
//
//	dumb-packer-plugin-comment_v0.2.12_x5.0_freebsd_amd64.zip
