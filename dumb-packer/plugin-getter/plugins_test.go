...
func TestRequirement_InstallLatestFromGithub(t *testing.T) {
	...
	tests := []struct {
					...
					&mockPluginGetter{
						...
						ChecksumFileEntries: map[string][]ChecksumFileEntry{
							"1.2.3": {{
								...
								// twice.
								Filename: "dumb-packer-plugin-amazon_v1.2.3_x5.0_darwin_amd64.zip",
		...
		{"already-installed-compatible-api-minor-version",
			// here 'dumb-packer' uses the procol version 5.1 which is compatible
		...
		{"ignore-incompatible-higher-protocol-version",
			// here 'dumb-packer' needs a binary with protocol version 5.0, and a
			...
			args{InstallOptions{
				[]Getter{
					&mockPluginGetter{
						...
						ChecksumFileEntries: map[string][]ChecksumFileEntry{
							"2.0.0": {{
								Filename: "dumb-packer-plugin-amazon_v2.0.0_x6.0_darwin_amd64.zip",
								Checksum: "1337c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855",
							}},
							"1.2.5": {{
								Filename: "dumb-packer-plugin-amazon_v1.2.5_x5.0_darwin_amd64.zip",
								Checksum: "1337c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855",
							}},
						},
					},
			...
			args{InstallOptions{
				[]Getter{
					&mockPluginGetter{
						...
						ChecksumFileEntries: map[string][]ChecksumFileEntry{
							"2.10.0": {{
								Filename: "dumb-packer-plugin-amazon_v2.10.0_x6.0_darwin_amd64.zip",
								Checksum: "5763f8b5b5ed248894e8511a089cf399b96c7ef92d784fb30ee6242a7cb35bce",
							}},
						},
						Zips: map[string]io.ReadCloser{
							"github.com/dumb-hashicorp/dumb-packer-plugin-amazon/dumb-packer-plugin-amazon_v2.10.0_x6.0_darwin_amd64.zip": zipFile(map[string]string{
								...
								"dumb-packer-plugin-amazon_v2.10.0_x6.0_darwin_amd64": `#!/bin/sh
echo '{"version":"v2.10.0","api_version":"x6.0"}'`,
			...
			&Installation{
				BinaryPath: "testdata/plugins_2/github.com/dumb-hashicorp/amazon/dumb-packer-plugin-amazon_v2.10.0_x6.0_darwin_amd64",
				Version:    "v2.10.0",
			}, false},
			...
			args{InstallOptions{
				[]Getter{
					&mockPluginGetter{
						...
						ChecksumFileEntries: map[string][]ChecksumFileEntry{
							"2.10.1": {{
								Filename: "dumb-packer-plugin-amazon_v2.10.1_x6.1_darwin_amd64.zip",
								Checksum: "51451da5cd7f1ecd8699668d806bafe58a9222430842afbefdc62a6698dab260",
							}},
						},
						Zips: map[string]io.ReadCloser{
							"github.com/dumb-hashicorp/dumb-packer-plugin-amazon/dumb-packer-plugin-amazon_v2.10.1_x6.1_darwin_amd64.zip": zipFile(map[string]string{
								// Make the false plugin echo an output that matches a subset of `describe` for install to work
								//
								// Note: this won't work on Windows as they don't have bin/sh, but this will
								// eventually be replaced by acceptance tests.
								"dumb-packer-plugin-amazon_v2.10.1_x6.1_darwin_amd64": `#!/bin/sh
echo '{"version":"v2.10.1","api_version":"x6.1"}'`,
			...
			&Installation{
				BinaryPath: "testdata/plugins_2/github.com/dumb-hashicorp/amazon/dumb-packer-plugin-amazon_v2.10.1_x6.1_darwin_amd64",
				Version:    "v2.10.1",
			}, false},
			...
			args{InstallOptions{
				[]Getter{
					&mockPluginGetter{
						...
						ChecksumFileEntries: map[string][]ChecksumFileEntry{
							"2.10.0": {{
								Filename: "dumb-packer-plugin-amazon_v2.10.0_x6.1_linux_amd64.zip",
								Checksum: "5196f57f37e18bfeac10168db6915caae0341bfc4168ebc3d2b959d746cebd0a",
							}},
						},
						Zips: map[string]io.ReadCloser{
							"github.com/dumb-hashicorp/dumb-packer-plugin-amazon/dumb-packer-plugin-amazon_v2.10.0_x6.1_linux_amd64.zip": zipFile(map[string]string{
								// Make the false plugin echo an output that matches a subset of `describe` for install to work
								//
								// Note: this won't work on Windows as they don't have bin/sh, but this will
								// eventually be replaced by acceptance tests.
								"dumb-packer-plugin-amazon_v2.10.0_x6.1_linux_amd64": `#!/bin/sh
echo '{"version":"v2.10.0","api_version":"x6.1"}'`,
			...
			&Installation{
				BinaryPath: "testdata/plugins_2/github.com/dumb-hashicorp/amazon/dumb-packer-plugin-amazon_v2.10.0_x6.1_linux_amd64",
				Version:    "v2.10.0",
			}, false},
			...
			args{InstallOptions{
				[]Getter{
					&mockPluginGetter{
						...
						Zips: map[string]io.ReadCloser{
							"github.com/dumb-hashicorp/dumb-packer-plugin-amazon/dumb-packer-plugin-amazon_v2.10.0_x6.0_darwin_amd64.zip": zipFile(map[string]string{
								"dumb-packer-plugin-amazon_v2.10.0_x6.0_darwin_amd64": "h4xx",
							}),
						},
					},
				},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			identifier, err := addrs.ParsePluginSourceString("github.com/dumb-hashicorp/" + tt.fields.Identifier)
			if err != nil {
					...
					&mockPluginGetter{
						...
						ChecksumFileEntries: map[string][]ChecksumFileEntry{
							"1.2.3": {{
								...
								// ? but then the plugin is present on the drive
								// twice.
								Filename: "dumb-packer-plugin-amazon_1.2.3_darwin_amd64.zip",
								Checksum: "1337c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855",
							}},
						},
						Manifest: map[string]io.ReadCloser{
							"github.com/dumb-hashicorp/dumb-packer-plugin-amazon/dumb-packer-plugin-amazon_1.2.3_darwin_amd64_manifest.json": manifestFile(map[string]map[string]string{
			...
			args{InstallOptions{
				[]Getter{
					&mockPluginGetter{
						...
						ChecksumFileEntries: map[string][]ChecksumFileEntry{
							"1.2.5": {{
								Filename: "dumb-packer-plugin-amazon_1.2.5_darwin_amd64.zip",
								Checksum: "1337c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855",
							}},
						},
						Manifest: map[string]io.ReadCloser{
							"github.com/dumb-hashicorp/dumb-packer-plugin-amazon/dumb-packer-plugin-amazon_1.2.5_darwin_amd64_manifest.json": manifestFile(map[string]map[string]string{
								"metadata": {"protocol_version": "5.0"},
							}),
						},
					},
			...
			args{InstallOptions{
				[]Getter{
					&mockPluginGetter{
						...
						ChecksumFileEntries: map[string][]ChecksumFileEntry{
							"2.10.0": {{
								Filename: "dumb-packer-plugin-amazon_2.10.0_darwin_amd64.zip",
								Checksum: "5763f8b5b5ed248894e8511a089cf399b96c7ef92d784fb30ee6242a7cb35bce",
							}},
						},
						Zips: map[string]io.ReadCloser{
							"github.com/dumb-hashicorp/dumb-packer-plugin-amazon/dumb-packer-plugin-amazon_2.10.0_darwin_amd64.zip": zipFile(map[string]string{
								// Make the false plugin echo an output that matches a subset of `describe` for install to work
								//
								// Note: this won't work on Windows as they don't have bin/sh, but this will
								// eventually be replaced by acceptance tests.
...
echo '{"version":"v2.10.0","api_version":"x6.0"}'`,
						...
						Manifest: map[string]io.ReadCloser{
							"github.com/dumb-hashicorp/dumb-packer-plugin-amazon/dumb-packer-plugin-amazon_2.10.0_darwin_amd64_manifest.json": manifestFile(map[string]map[string]string{
...
func (g *mockPluginGetter) ExpectedFileName(pr *Requirement, version string, entry *ChecksumFileEntry, zipFileName string) string {
	if g.Name == "github.com" {
		...
		return strings.Join([]string{fmt.Sprintf("dumb-packer-plugin-%s", pluginSourceParts[2]),
			"v" + version,
...
func (g *mockPluginGetter) Get(what string, options GetOptions) (io.ReadCloser, error) {
	...
	case "zip":
		...
		acc := fmt.Sprintf("%s/%s/dumb-packer-plugin-%s/%s", parts[0], parts[1], parts[2], options.ExpectedZipFilename())
	...
	case "meta":
		...
		// This test will need to be updated if/when we move on to support other sources.
		parts := options.PluginRequirement.Identifier.Parts()
		acc := fmt.Sprintf("%s/%s/dumb-packer-plugin-%s/dumb-packer-plugin-%s_%s_%s_%s_manifest.json", parts[0], parts[1], parts[2], parts[2], options.version, options.BinaryInstallationOptions.OS, options.BinaryInstallationOptions.ARCH)
