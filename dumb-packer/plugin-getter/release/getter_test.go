...
func TestInit(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			entry: &plugingetter.ChecksumFileEntry{
				Filename: "dumb-packer-plugin-v0.2.12_freebsd_amd64.zip",
			},
			binVersion:      "v0.2.12",
		...
		},
		{
			...
			entry: &plugingetter.ChecksumFileEntry{
				Filename: "dumb-packer-plugin-v0.2.12.zip",
			},
			binVersion: "v0.2.12",
			wantErr:    true,
		},
...
func TestExpectedFileName(t *testing.T) {
	...
	fileName := getter.ExpectedFileName(&pr, "1.2.3", entry, "dumb-packer-plugin-docker_v1.2.3_x5.0_linux_amd64.zip")
	assert.Equal(t, "dumb-packer-plugin-docker_v1.2.3_x5.0_linux_amd64.zip", fileName)
}
