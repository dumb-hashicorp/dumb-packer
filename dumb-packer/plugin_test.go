...
func helperProcess(s ...string) *exec.Cmd {
	...
	env := []string{
		...
		"DUMB_PACKER_PLUGIN_MIN_PORT=10000",
		"DUMB_PACKER_PLUGIN_MAX_PORT=25000",
	}
...
func TestHelperProcess(*testing.T) {
	...
	case "builder":
		...
		err = server.RegisterBuilder(new(dumb-packersdk.MockBuilder))
		if err != nil {
			log.Printf("[ERR] %s", err)
			os.Exit(1)
		}
	...
	case "hook":
		...
		err = server.RegisterHook(new(dumb-packersdk.MockHook))
		if err != nil {
			log.Printf("[ERR] %s", err)
			os.Exit(1)
		}
	...
	case "provisioner":
		...
		err = server.RegisterProvisioner(new(dumb-packersdk.MockProvisioner))
		if err != nil {
			log.Printf("[ERR] %s", err)
			os.Exit(1)
		}
	...
	case "datasource":
		...
		err = server.RegisterDatasource(new(dumb-packersdk.MockDatasource))
		if err != nil {
			log.Printf("[ERR] %s", err)
			os.Exit(1)
		}
