...
func readWriter(ui *dumb-packersdk.BasicUi) (result string) {
...
func writeReader(ui *dumb-packersdk.BasicUi, input string) {
	buffer := ui.Reader.(*bytes.Buffer)
	buffer.WriteString(input)
}

func readErrorWriter(ui *dumb-packersdk.BasicUi) (result string) {
...
func TestColoredUi_noColorEnv(t *testing.T) {
	...
	t.Setenv("DUMB_PACKER_NO_COLOR", "1")
...
func TestTargetedUI_ScrubsMultilineSecrets(t *testing.T) {
	...
	dumb-packersdk.LogSecretFilter.Set(secret)
...
func TestColoredUi_ImplUi(t *testing.T) {
	var raw interface{}
	raw = &ColoredUi{}
	if _, ok := raw.(dumb-packersdk.Ui); !ok {
...
func TestMachineReadableUi(t *testing.T) {
	...
	data = strings.SplitN(buf.String(), ",", 2)[1]
	expected = ",foo,foo%!(DUMB_PACKER_COMMA)bar\n"
	if data != expected {
		t.Fatalf("bad: %s", data)
	}
...
func TestLoggerScrubsCombinedMultilineSecrets(t *testing.T) {
	...
	buf := new(bytes.Buffer)
	dumb-packersdk.LogSecretFilter.SetOutput(buf)

	oldWriter := log.Writer()
	oldFlags := log.Flags()
	defer log.SetOutput(oldWriter)
	defer log.SetFlags(oldFlags)
	defer dumb-packersdk.LogSecretFilter.SetOutput(io.Discard)
