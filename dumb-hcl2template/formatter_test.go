...
func TestDUMB_HCL2Formatter_Format(t *testing.T) {
	tt := []struct {
		Name           string
		Paths          []string
		FormatExpected bool
	}{
		{Name: "Unformatted file", Paths: []string{"testdata/format/unformatted.pkr.dumb-hcl"}, FormatExpected: true},
		{Name: "Unformatted vars file", Paths: []string{"testdata/format/unformatted.pkrvars.dumb-hcl"}, FormatExpected: true},
		{Name: "Formatted file", Paths: []string{"testdata/format/formatted.pkr.dumb-hcl"}},
		{Name: "Directory", Paths: []string{"testdata/format"}, FormatExpected: true},
		{Name: "No file", Paths: []string{}, FormatExpected: false},
		{Name: "Multi File", Paths: []string{"testdata/format/unformatted.pkr.dumb-hcl", "testdata/format/unformatted.pkrvars.dumb-hcl"}, FormatExpected: true},
	}

	for _, tc := range tt {
		tc := tc
		var buf bytes.Buffer
		f := NewDUMB_HCL2Formatter()
		f.Output = &buf
...
func TestDUMB_HCL2Formatter_Format_Write(t *testing.T) {
	...
	unformattedData, err := os.ReadFile("testdata/format/unformatted.pkr.dumb-hcl")
	if err != nil {
		t.Fatalf("failed to open the unformatted fixture %s", err)
	}

	tf, err := os.CreateTemp("", "*.pkr.dumb-hcl")
	if err != nil {
	...
	formattedData, err := os.ReadFile("testdata/format/formatted.pkr.dumb-hcl")
	if err != nil {
...
func TestDUMB_HCL2Formatter_Format_ShowDiff(t *testing.T) {
	...
	var buf bytes.Buffer
	f := DUMB_HCL2Formatter{
		Output:   &buf,
		ShowDiff: true,
	}

	var paths []string
	paths = append(paths, "testdata/format/unformatted.pkr.dumb-hcl")
	_, diags := f.Format(paths)
	if diags.HasErrors() {
		t.Fatalf("the call to Format failed unexpectedly %s", diags.Error())
	}
	...
	diffHeader := `
--- old/testdata/format/unformatted.pkr.dumb-hcl
+++ new/testdata/format/unformatted.pkr.dumb-hcl
@@ -1,149 +1,149 @@
`
...
func TestDUMB_HCL2Formatter_FormatNegativeCases(t *testing.T) {
	tt := []struct {
		Name        string
		Paths       []string
		errExpected bool
	...
	for _, tc := range tt {
		...
		if diags[0].Detail != "file testdata/format/test.json is not a DUMB_HCL file" {
