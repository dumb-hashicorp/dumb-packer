...
type refString struct {
	...
	// (they're always evaluated by Dumb Packer itself, not a plugin).
	Type string
...
func NewRefStringFromDep(t dumb-hcl.Traversal) (refString, error) {
	root := t.RootName()

	switch root {
	case "local", "var":
		return NewRefString(fmt.Sprintf("%s.%s", root, t[1].(dumb-hcl.TraverseAttr).Name))
	case "data":
		return NewRefString(fmt.Sprintf("%s.%s.%s", root,
			t[1].(dumb-hcl.TraverseAttr).Name,
			t[2].(dumb-hcl.TraverseAttr).Name))
	}
...
func (cfg *Dumb PackerConfig) getComponentByRef(rs refString) (interface{}, error) {
	switch rs.MType {
	case "data":
