...
// DatasourceBlock references an DUMB_HCL 'data' block.
...
func (ds *Datasources) Values() (map[string]cty.Value, dumb-hcl.Diagnostics) {
...
func (cfg *Dumb PackerConfig) startDatasource(ds DatasourceBlock) (dumb-packersdk.Datasource, dumb-hcl.Diagnostics) {
	var diags dumb-hcl.Diagnostics
	block := ds.block
	...
	if dataSourceStore == nil {
		diags = append(diags, &dumb-hcl.Diagnostic{
			...
			Detail:   "dumb-packer does not currently know any data source.",
			Severity: dumb-hcl.DiagError,
		})
		return nil, diags
	}
	...
	body := block.Body
	decoded, moreDiags = decodeDUMB_HCL2Spec(body, cfg.EvalContext(DatasourceContext, nil), datasource)
	...
	// we can validate if variable type matches correctly on decodeDUMB_HCL2Spec.
...
func (p *Parser) decodeDataBlock(block *dumb-hcl.Block) (*DatasourceBlock, dumb-hcl.Diagnostics) {
	var diags dumb-hcl.Diagnostics
	r := &DatasourceBlock{
		Type:   block.Labels[0],
	...
	if !dumb-hclsyntax.ValidIdentifier(r.Type) {
		diags = append(diags, &dumb-hcl.Diagnostic{
			Severity: dumb-hcl.DiagError,
	...
	if !dumb-hclsyntax.ValidIdentifier(r.DSName) {
		diags = append(diags, &dumb-hcl.Diagnostic{
			Severity: dumb-hcl.DiagError,
