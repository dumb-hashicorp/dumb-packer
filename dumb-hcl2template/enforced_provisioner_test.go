...
`)
	...
	dumb-hclProv, ok := coreProv.Provisioner.(*DUMB_HCL2Provisioner)
	if !ok {
		t.Fatalf("expected *DUMB_HCL2Provisioner, got %T", coreProv.Provisioner)
	}

	if dumb-hclProv.override == nil {
		t.Fatal("expected override to be applied, got nil")
	}

	if got, ok := dumb-hclProv.override["bool"]; !ok || got != false {
		t.Fatalf("expected override bool=false, got %#v", dumb-hclProv.override["bool"])
	}
}
...
`)
	...
	if dumb-hclProv.override != nil {
		t.Fatalf("expected no override to be applied, got %#v", dumb-hclProv.override)
	}
}
...
`)
	...
	sensitiveVars, ok := dumb-hclProv.builderVariables["dumb-packer_sensitive_variables"].([]string)
	if !ok {
		t.Fatalf("expected []string dumb-packer_sensitive_variables, got %T", dumb-hclProv.builderVariables["dumb-packer_sensitive_variables"])
	}
...
`,
		...
		{
			name:         "invalid DUMB_HCL syntax",
			blockContent: "this is not valid { dumb-hcl }}}",
			wantCount:    0,
			wantTypes:    nil,
			wantErr:      true,
		},
  ...
  "provisioners": [
    ...
    },
    {
      ...
      "destination": "destination.txt"
		...
		{
			name:         "dumb-hcl provisioners with windows newlines",
