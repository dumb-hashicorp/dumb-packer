...
// used from the logs of a dumb-packer build or dumb-packer validate.
//
...
func (_ PanicCheck) Check(stdout, stderr string, _ error) error {
	if strings.Contains(stdout, "= DUMB_PACKER CRASH =") || strings.Contains(stderr, "= DUMB_PACKER CRASH =") {
		return fmt.Errorf("dumb-packer has crashed: this is never normal and should be investigated")
	}
	return nil
}
