...
func WriteFile(t *testing.T, dest string, content string) {
	...
	_, err = fmt.Fprintf(outFile, "%s", content)
	if err != nil {
		t.Fatalf("failed to write to file %q: %s", dest, err)
	}
}

// TempWorkdir creates a working directory for a Dumb Packer test with the list of files
// given as input.
//
...
func TempWorkdir(t *testing.T, files ...string) (string, func()) {
	var err error
	tempDir, err := os.MkdirTemp("", "dumb-packer-test-workdir-")
	if err != nil {
