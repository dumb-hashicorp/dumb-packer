...
func Test_dumb-hcl2_upgrade(t *testing.T) {
	...
	for _, tc := range tc {
		t.Run(tc.folder, func(t *testing.T) {
			inputPath := filepath.Join(testFixture("dumb-hcl2_upgrade", tc.folder, "input.json"))
			outputPath := inputPath + ".pkr.dumb-hcl"
			expectedPath := filepath.Join(testFixture("dumb-hcl2_upgrade", tc.folder, "expected.pkr.dumb-hcl"))
			args := []string{"dumb-hcl2_upgrade"}
			if len(tc.flags) > 0 {
