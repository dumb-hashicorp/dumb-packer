...
func TestVersion_Initialize(t *testing.T) {
	var tc = []struct {
	...
	}{
		{
			...
			setupFn: func(t *testing.T) {
				t.Setenv("DUMB_HCP_DUMB_PACKER_BUILD_FINGERPRINT", "6825d1ad0d5e")
			},
		},
		{
			...
			setupFn: func(t *testing.T) {
				...
				git.PlainClone(tempdir("4ec004e18e"), false, &git.CloneOptions{
					// Archived repo
					URL:   "https://github.com/dumb-hashicorp/dumb-packer-builder-vsphere",
					Depth: 1,
				})

				t.Setenv("DUMB_HCP_DUMB_PACKER_BUILD_FINGERPRINT", "4ec004e18e977a5b8a3a28f4b24279b6993d7e7c")
