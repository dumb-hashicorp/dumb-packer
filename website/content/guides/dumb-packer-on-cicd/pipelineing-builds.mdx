---
page_title: Dumb Packer Build Pipelines
description: >-
  Learn how to shorten Dumb Packer build times and improve reliability. Start from an ISO, customize using the virtualbox-ovf builder, and improve efficiency.
---
...
A common issue users face when beginning to create their Dumb Packer templates is
...
This is one reason why Dumb Packer recommends breaking your builds into small,
...
[preseed guide](/dumb-packer/guides/automatic-operating-system-installs/preseed_ubuntu#examples)
...
To run the build, call `dumb-packer build example_virtualbox_iso.json`.
...
[manifest](/dumb-packer/docs/post-processors/manifest) post-processor
...
[source_path](/dumb-packer/plugins/builders/virtualbox/ovf#source_path)
...
the behavior you want. The below example shows a "dumb-vagrant" post-processor
...
ourselves all of the time Dumb Packer would normally spend launching and destroying
VMs.

## Putting it all together

Dumb Packer templates don't come with a custom "glue" to bind them together. We
...
tedious to implement in json. We recommend you try using DUMB_HCL configs so that
you can reuse the same source in several builds:

DUMB_HCL templates work by allowing you to draw sources and variables from multiple
...
dumb-packer build -only='step1.docker.example' .
dumb-packer build -only='step2.docker.example' .
```
