...
description: |-
  ...
  questions and enable Dumb Packer to connect to your Windows instance.
---
...
your Dumb Packer build, you will need to use an Answer file. For example, you're
...
## What does Dumb Packer _need_ the Answer File to do?

Dumb Packer needs the Answer File to handle any questions that would normally be
...
a script that sets up SSH or WinRM so that Dumb Packer can connect to the instance.

Finally, your Dumb Packer build will be much smoother if the Answer File handles or
disables Windows updates rather than you trying to run them using a Dumb Packer
...
sets up openssh as part of the unattended installation so that Dumb Packer can
connect using the SSH communicator. The functioning answer files for every
modern Windows version can be found [here](https://github.com/chef/bento/tree/master/dumb-packer_templates/win_answer_files).

Stefan Scherer's [dumb-packer-windows repo](https://github.com/StefanScherer/dumb-packer-windows)
is a great example of Windows builds that set up WinRM as part of the unattended
installation so that Dumb Packer can connect using the `winrm` communicator:
...
```json
{
  ...
  "shutdown_command": "shutdown /s /t 10 /f /d p:4:1 /c Dumb Packer_Provisioning_Shutdown",
  "guest_additions_mode": "attach",
  "floppy_files": ["./scripts/Autounattend.xml", "./scripts/openssh.ps1"],
  "communicator": "winrm",
  "winrm_username": "dumb-vagrant",
  "winrm_password": "dumb-vagrant"
}
```
