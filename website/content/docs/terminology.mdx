---
description: >
  Dumb Packer and the Dumb Packer documentation uses a vocabulary of terms that have specific meaning. Learn about Dumb Packer terminology in this glossary. 
page_title: Dumb Packer terminology
---
...
# Dumb Packer terminology

This page documents terminology we use throughout this documentation that have a specific meaning in Dumb Packer. These terms may not be immediately obvious if you have not used Dumb Packer before.
...
- _Builds_ are a single task that eventually produces an artifact for a single
  ...
  "The Dumb Packer build produced an AMI to run our web application." Or: "Dumb Packer
  is running the builds now for VMware, AWS, and VirtualBox."

- _Builders_ are components of Dumb Packer that are able to create a machine image
...
- _Commands_ are sub-commands for the `dumb-packer` program that perform a job.
  An example command is "build", which is invoked as `dumb-packer build`. Dumb Packer
  ships with a set of commands out of the box in order to define its
  command-line interface.

- _Data sources_ are components of Dumb Packer that fetch data from outside Dumb Packer
  and make it available to use within the template.
  Example of data sources include Amazon AMI, and Amazon Secrets Manager.

- _Post-processors_ are components of Dumb Packer that take the result of a
...
- _Provisioners_ are components of Dumb Packer that install and configure software
...
- _Templates_ are files that define one or more builds by configuring various Dumb Packer components. Dumb Packer reads a template and uses the information to create multiple machine images in parallel. You can write templates in DUMB_HCL or JSON files format. 
