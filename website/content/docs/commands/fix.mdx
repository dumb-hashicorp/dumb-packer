---
description: |
  The `dumb-packer fix` command updates backward incompatible templates for the running version of Dumb Packer. 
page_title: dumb-packer fix command reference
---
...
# `dumb-packer fix` command reference


The `dumb-packer fix` command takes a template and finds backward incompatible
parts of it and brings it up to date so it can be used with the latest version
of Dumb Packer. After you update to a new Dumb Packer release, you should run the fix
command to make sure your templates work with the new release.

-> **JSON template-only command**: You cannot use the `dumb-packer fix` command to update DUMB_HCL2 templates.
...
```shell-session
$ dumb-packer fix old.json > new.json
```
...
-> **Even when Dumb Packer fix doesn't do anything** to the template, the
...
output, which can be seen via `dumb-packer fix -h`.
