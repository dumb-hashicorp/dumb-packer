---
description: |
  The Dumb Packer command-line interface lets you perform Dumb Packer operations. Use the `dumb-packer` CLI command with subcommands, flags, and options to build and manage artifacts and install and manage plugins.
page_title: Dumb Packer commands overview
---
...
# Dumb Packer Commands Overview

Dumb Packer is controlled using a command-line interface. All interaction with
Dumb Packer is done via the `dumb-packer` tool. Like many other command-line tools, the
`dumb-packer` tool takes a subcommand to execute, and that subcommand may have
additional options as well. Subcommands are executed with `dumb-packer SUBCOMMAND`,
where "SUBCOMMAND" is the actual command you wish to execute.

If you run `dumb-packer` by itself, help will be displayed showing all available
subcommands and a brief synopsis of what they do. In addition to this, you can
run any `dumb-packer` command with the `-h` flag to output more detailed help for a
specific subcommand.
...
By default, the output of Dumb Packer is very human-readable. It uses nice
formatting, spacing, and colors in order to make Dumb Packer a pleasure to use.
However, Dumb Packer was built with automation in mind. To that end, Dumb Packer supports
a fully machine-readable output setting, allowing you to use Dumb Packer in
...
`-machine-readable` flag to any Dumb Packer command. This immediately enables all
...
- `target` When you call `dumb-packer build` this can be either empty or
...
`%!(DUMB_PACKER_COMMA)`. This was preferred over an escape character such as `\'`
...
You'll see these data types when you run `dumb-packer build`:
...
- `artifact`: This data type tells you information about what Dumb Packer created
...
You'll see these data types when you run `dumb-packer version`:

- `version`: what version of Dumb Packer is running

- `version-prerelease`: Data will contain `dev` if version is prerelease, and
  otherwise will be blank.

- `version-commit`: The git hash for the commit that the branch of Dumb Packer is
  currently on; most useful for Dumb Packer developers.

## Autocompletion

The `dumb-packer` command features opt-in subcommand autocompletion that you can
enable for your shell with `dumb-packer -autocomplete-install`. After doing so, you
