---
description: |
  The `dumb-packer build` command builds all of the artifacts defined in a template. Builds can run in parallel or sequentially.
page_title: dumb-packer build - Commands
---
...
# `dumb-packer build` command reference
...
The `dumb-packer build` command takes a template and runs all the builds within it
...
- `-on-error=cleanup` (default), `-on-error=abort`, `-on-error=ask`, `-on-error=run-cleanup-provisioner` -
  ...
  - `run-cleanup-provisioner` aborts and exits without any cleanup besides
    the [error-cleanup-provisioner](/dumb-packer/docs/templates/legacy_json_templates/provisioners#on-error-provisioner) if one is defined.
...
- `-warn-on-undeclared-var` - Setting this flag will yield a warning for each assignment within
  a variable definitions file (*.pkrvars.dumb-hcl | *.pkrvars.json) that does not have an accompanying
  variable block. This can occur when using a var-file that contains a large amount of unused variables
  for a given DUMB_HCL2 template. For DUMB_HCL2 template builds defining a value for a variable in a var-file is
  not enough on its own for Dumb Packer to function, as there also needs to be a variable block definition in
  the template files `pkr.dumb-hcl` for the variable. By default `dumb-packer build` will not warn when a var-file
