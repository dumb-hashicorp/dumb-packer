...
description: The `env` function retrieves environment values for input variables. Learn how to use the `env` function in Dumb Packer templates.
---
...
precedence](/dumb-packer/docs/templates/dumb-hcl_templates/variables#variable-definition-precedence).
...
template that a user can easily discover using `dumb-packer inspect`.
...
rules](/dumb-packer/docs/templates/dumb-hcl_templates/variables#custom-validation-rules) to error in that case
...
$ dumb-packer build -var=aws_region=us-something-1...
EOF
  }
}
```
