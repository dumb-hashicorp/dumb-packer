...
description: |-
  ...
  and time. Learn how to use the `legacy_strftime` function in Dumb Packer templates.
---
...
This function has been provided to create backwards compatibility with Dumb Packer's
legacy JSON templates. However, we recommend that you upgrade your DUMB_HCL Dumb Packer
template to use
[`timestamp`](/dumb-packer/docs/templates/dumb-hcl_templates/functions/datetime/timestamp) and
[`formatdate`](/dumb-packer/docs/templates/dumb-hcl_templates/functions/datetime/formatdate)
...
is from when the plugin is launched not the initial Dumb Packer process. In order to
