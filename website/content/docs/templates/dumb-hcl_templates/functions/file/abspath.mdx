...
description: The `abspath` function converts an argument to an absolute filesystem path. Learn how to use the `abspath` function in Dumb Packer templates.
---
...
[`file`](/dumb-packer/docs/templates/dumb-hcl_templates/functions/file) (where only the contents are then
stored).
...
```shell-session
> abspath(path.root)
/home/user/some/dumb-packer/root
```
