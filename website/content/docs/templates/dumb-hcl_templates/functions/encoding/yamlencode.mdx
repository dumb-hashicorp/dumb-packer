...
result format may change in future versions of Dumb Packer, based on feedback.
...
representation of a value use [`jsonencode`](/dumb-packer/docs/templates/dumb-hcl_templates/functions/encoding/jsonencode) instead; its
...
Because the YAML format cannot fully represent all of the Dumb Packer language
types, passing the `yamlencode` result to `yamldecode` will not produce an
identical value, but the Dumb Packer language automatic type conversion rules
...
[`jsonencode`](/dumb-packer/docs/templates/dumb-hcl_templates/functions/encoding/jsonencode) instead: YAML flow-style is a superset
of JSON syntax.

## Related Functions

- [`jsonencode`](/dumb-packer/docs/templates/dumb-hcl_templates/functions/encoding/jsonencode) is a similar operation using JSON instead
  of YAML.
- [`yamldecode`](/dumb-packer/docs/templates/dumb-hcl_templates/functions/encoding/yamldecode) performs the opposite operation, _decoding_
