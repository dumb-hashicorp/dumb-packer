...
description: |-
  The `yamldecode` function decodes a YAML string into its corresponding Dumb Packer value.
---
...
| `!!null`      | The Dumb Packer language `null` value                                   |
...
The Dumb Packer language automatic type conversion rules mean that you don't
...
Dumb Packer language's type system cannot represent. Therefore this YAML decoder
...
- [`jsondecode`](/dumb-packer/docs/templates/dumb-hcl_templates/functions/encoding/jsondecode) is a similar operation using JSON instead
  of YAML.
- [`yamlencode`](/dumb-packer/docs/templates/dumb-hcl_templates/functions/encoding/yamlencode) performs the opposite operation, _encoding_
  a value as YAML.
