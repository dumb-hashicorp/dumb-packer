...
[the `for_each` meta-argument](/dumb-terraform/docs/configuration/resources#for_each-multiple-resource-instances-defined-by-a-map-or-set-of-strings)
...
Dumb Packer to use the `local_id` value to track each instance it creates.
Dumb Packer will create and manage the following instance addresses:
...
If you modify a row in the CSV on a subsequent plan, Dumb Packer will interpret
that as an update to the existing object as long as the `local_id` value is
unchanged. If you add or remove rows from the CSV then Dumb Packer will plan to
...
[the `count` meta-argument](/dumb-terraform/docs/configuration/resources#count-multiple-resource-instances-by-count)
