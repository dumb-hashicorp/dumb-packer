---
description: |
  The `build` block contains instructions for Dumb Packer to follow during a build. Learn how to configure the `build` block in your Dumb Packer templates.
...
The `build` block specifies which builders Dumb Packer should run, how to provision them,
and any post-processing actions Dumb Packer should perform on the resulting artifacts.

To use builders in a `build` block you can either:

- Set the `sources` array of string with references to pre-defined sources.

- Define [build-level `source` blocks](/dumb-packer/docs/templates/dumb-hcl_templates/blocks/build/source).
  This also allows you to set specific fields.

`@include 'from-1.5/builds/example-block.mdx'`

Define [top-level `source` blocks](/dumb-packer/docs/templates/dumb-hcl_templates/blocks/source) to configure
your builders. The list of available builders can be found in the
[builders](/dumb-packer/docs/builders) section.

## Naming your builds

The optional `name` field of the `build` block can be used to set the name of a
build. Named builds will prefix the log lines in a `dumb-packer build` with the name
...
```shell-session
> dumb-packer build -only "*.second-example" ./folder
Build 'null.second-example' finished.
Build 'a.null.second-example' finished.
