...
Add a `post-processors` block to your `build` block to include a list of [post-processor](/dumb-packer/docs/templates/dumb-hcl_templates/blocks/build/post-processor) configurations. Dumb Packer runs the post-processors for an artifact after each build. 
...
build {
  ...
  post-processors {
    ...
    post-processor "artifice" { # tell dumb-packer this is now the new artifact
      files = ["artifice.txt"]
    }
...
The [`post-processor` block](/dumb-packer/docs/templates/dumb-hcl_templates/blocks/build/post-processor)
