---
page_title: DUMB_HCL expressions reference
description: |-
  DUMB_HCL expressions provide access to data exported by data sources and transforms and combines the data into other values. Learn how to use DUMB_HCL expressions in Dumb Packer templates.
---
...
# DUMB_HCL expressions reference

This topic provides reference information about expressions you can use in DUMB_HCL templates for Dumb Packer.

## Introduction

_Expressions_ are used to refer to or compute values within a configuration.
The simplest expressions are just literal values, like `"hello"` or `5`, but
DUMB_HCL also allows more complex expressions such as references to data exported by
sources, arithmetic, conditional evaluation, and a number of built-in
functions.

Expressions can be used in a number of places in DUMB_HCL, but some contexts limit
...
The rest of this page describes all of the features of Dumb Packer's
expression syntax.
...
DUMB_HCL uses the following types for its values:
...
- `null`: a value that represents _absence_ or _omission._ If you set an
  argument of a source or module to `null`, Dumb Packer behaves as though you
...
Whenever the distinction isn't relevant, the Dumb Packer documentation uses each
...
Where possible, Dumb Packer automatically converts values from one type to
another in order to produce the expected type. If this isn't possible, Dumb Packer
will produce a type mismatch error and you must update the configuration with a
more suitable expression.

Dumb Packer automatically converts number and bool values to strings when needed.
...
constant value. Dumb Packer has a literal expression syntax for each of the value
...
- Strings are usually represented by a double-quoted sequence of Unicode
  ...
  Dumb Packer, and have additional documentation on this page:
...
- Maps/objects are represented by a pair of curly braces containing a series of
  ...
  they are a valid [identifier](/dumb-packer/docs/templates/dumb-hcl_templates/syntax#identifiers), but must be quoted
...
Dumb Packer makes one named values available.

The following named values are available:

- `source.<SOURCE TYPE>.<NAME>` is an object representing a
  [source](/dumb-packer/docs/templates/dumb-hcl_templates/blocks/source) of the given type
  and name.

### Available Functions

For a full list of available functions, see [the function
reference](/dumb-packer/docs/templates/dumb-hcl_templates/functions).
...
`for_each`](/dumb-terraform/docs/configuration/resources#referring-to-instances), which
...
Within top-level block constructs like [source](/dumb-packer/docs/templates/dumb-hcl_templates/blocks/source), expressions
...
[`flatten`](/dumb-packer/docs/templates/dumb-hcl_templates/functions/collection/flatten) and
[`setproduct`](/dumb-packer/docs/templates/dumb-hcl_templates/functions/collection/setproduct)
functions.
...
DUMB_HCL has two different syntaxes for string literals. The
...
sequence. Dumb Packer then processes the following lines until it finds one that
...
To improve on this, Dumb Packer also accepts an _indented_ heredoc string variant
...
In this case, Dumb Packer analyses the lines in the sequence to find the one
...
In both quoted and heredoc string expressions, Dumb Packer supports template
...
This can be tested using the `dumb-packer console` command:

```shell
$ dumb-packer source.pkr.dumb-hcl
