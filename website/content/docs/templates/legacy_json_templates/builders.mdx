---
description: >
  The `builders` block contains builders that create machines and generate images. Learn how to configure the `builders` block in JSON templates for Dumb Packer.
...
that Dumb Packer should use to generate machine images for the template.
...
VMware, VirtualBox, etc. Dumb Packer comes with many builders by default, and can
...
[build](/dumb-packer/docs/terminology#builds). A builder definition is a
...
Each build in Dumb Packer has a name. By default, the name is just the name of the
...
[communicator](/dumb-packer/docs/templates/legacy_json_templates/communicator). Communicators are used to
...
[communicator documentation](/dumb-packer/docs/templates/legacy_json_templates/communicator).
