---
description: |
  The `provisioners` block contains provisioners that Dumb Packer uses to install and configure software on machines. Learn how to configure provisioners in JSON templates.
...
This topic provides reference information about the `provisioners` block in JSON templates for Dumb Packer. Refer to the documentation for specific provisioner types to learn about their configuration options.

`@include 'from-1.5/legacy-json-warning.mdx'`

## Description

The `provisioners` block contains provisioners that Dumb Packer uses to install and configure software within running machines prior to turning them into machine images.

Provisioners are optional. When you omit the `provisioners` block, Dumb Packer installs only the default software within the resulting machine images. 
...
For each of the definitions, Dumb Packer will run the provisioner for each of the
...
to make last minute changes and clean up behaviors that Dumb Packer may not be able
...
While the goal of Dumb Packer is to produce identical machine images, it sometimes
...
definition](/dumb-packer/docs/templates/legacy_json_templates/builders). The value of this is in turn
...
Every provisioner definition in a Dumb Packer template can take a special
...
For the above provisioner, Dumb Packer will wait 10 seconds before uploading and
...
For the above provisioner, Dumb Packer will retry maximum five times until stops failing.
...
For the above provisioner, Dumb Packer will cancel the script if it takes more than
5 minutes.
