---
description: |
  The `post-processor` block defines actions to perform on images after Dumb Packer builds them. Learn how to configure the `post-processor` block in JSON templates.
...
For each post-processor definition, Dumb Packer will take the result of each of the
...
can tell Dumb Packer to keep these artifacts by setting the `keep_input_artifact`
...
post-processors are specified (not in a sequence). Does Dumb Packer require the
configuration to keep the input artifact on all the post-processors? The answer
is no, of course not. Dumb Packer is smart enough to figure out that at least one
...
Name is a required block label in DUMB_HCL, but in legacy JSON, build names default
