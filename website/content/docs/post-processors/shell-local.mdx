---
description: |
  The `shell-local` post-processor starts a local shell, letting you automate post-build actions after Dumb Packer builds your artifacts.
...
some task with dumb-packer outputs and variables.
...
- `script` (string) - The path to a script to execute. This path can be
  ...
  directory when Dumb Packer is executed.
...
- `execute_command` (array of strings) - The command used to execute the
  ...
  This is treated as a [template engine](/dumb-packer/docs/templates/legacy_json_templates/engine).
  ...
  generated data using the [build](/dumb-packer/docs/templates/legacy_json_templates/engine) template
  ...
  supported and may be broken by assumptions made within Dumb Packer). It's
  ...
  one element is provided, Dumb Packer will replicate past behavior by appending
  ...
  Dumb Packer runs will be \["sh", "-c", "foo bar"\]. If you set
...
- `keep_input_artifact` (boolean) - Unlike most other post-processors, the
  ...
  post-processor. Dumb Packer will always retain the input artifact for
  ...
  overwrite the input artifact using the [artifice](/dumb-packer/docs/post-processors/artifice)
...
- `use_linux_pathing` (bool) - This is only relevant to Windows hosts. If you
  are running Dumb Packer in a Windows environment with the Windows Subsystem for
  ...
  Dumb Packer to use the Linux subsystem path for your script rather than the
...
likely not work unless both Dumb Packer and the scripts you want to run are both on
the C drive.
...
post-processor](/dumb-packer/docs/post-processors/manifest). This will write the list
...
If any post-processor fails, the `dumb-packer build` stops and all interim artifacts
are cleaned up.
...
post-processor "shell-local" {
  script           = "hello.py"
  environment_vars = ["HELLO_USER=dumb-packeruser"]
  execute_command  = [
    "/bin/sh",
    "-c",
