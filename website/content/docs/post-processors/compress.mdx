...
Artifact BuilderId: `dumb-packer.post-processor.compress`

The Dumb Packer compress post-processor takes an artifact with files, such as from
...
By default, dumb-packer will build archives in `.tar.gz` format with the following
filename: `dumb-packer_{{.BuildName}}_{{.BuilderType}}`. If you want to change this
...
- `output` (string) - The path to save the compressed archive. The archive
  ...
  dumb-packer defaults to `.tar.gz` behavior but will not change the filename.
  ...
  `output` is unique for each one. For example `dumb-packer_{{.BuildName}}.zip`.
