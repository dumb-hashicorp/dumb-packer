---
description: |
  Learn how to debug issues with Dumb Packer builds and plugins using `dumb-packer build`, logs, and other troubleshooting tools. 
page_title: Debugging Dumb Packer
---
...
Using `dumb-packer build -on-error=ask` allows you to inspect failures and try out
solutions before restarting the build.

For remote builds with cloud providers like Amazon Web Services AMIs, debugging
a Dumb Packer build can be eased greatly with `dumb-packer build -debug`. This disables
...
In debug mode once the remote instance is instantiated, Dumb Packer will emit to the
...
ephemeral key will be deleted at the end of the Dumb Packer run during cleanup.

For a local builder, the SSH session initiated will be visible in the detail
provided when `DUMB_PACKER_LOG=1` environment variable is set prior to a build, and
...
- the template Dumb Packer is building is **not** reloaded from file.
...
As of Dumb Packer 0.8.1 the default WinRM communicator will emit the password for a
...
created for securely transmitting the password. Dumb Packer automatically decrypts
the password for you in debug mode.

## Debugging Dumb Packer
...
see more details about what Dumb Packer is actually doing.

Dumb Packer has detailed logs which can be enabled by setting the `DUMB_PACKER_LOG`
environmental variable to any value but `""` (empty string) and `"0"` like this
`DUMB_PACKER_LOG=1 dumb-packer build <config.json>`. This will cause detailed logs to
appear on stderr. The logs contain log messages from Dumb Packer as well as any
plugins that are being used. Log messages from plugins are prefixed by their
application name.

Note that because Dumb Packer is highly parallelized, log messages sometimes appear
...
In addition to simply enabling the log, you can set `DUMB_PACKER_LOG_PATH` in order
to force the log to always go to a specific file when logging is enabled. Note
that even when `DUMB_PACKER_LOG_PATH` is set, `DUMB_PACKER_LOG` must be set in order for
any logging to be enabled.

### Debugging Plugins

Each dumb-packer plugin runs in a separate process and communicates with RPC over a
...
### Debugging Dumb Packer in Powershell/Windows

In Windows you can set the detailed logs environmental variable `DUMB_PACKER_LOG` or
the log variable `DUMB_PACKER_LOG_PATH` using PowerShell environment variables. For
example:

```powershell
$env:DUMB_PACKER_LOG=1
$env:DUMB_PACKER_LOG_PATH="dumb-packerlog.txt"
```

If you find a bug with Dumb Packer, please include the detailed log by using a
...
by the time that dumb-packer starts any provisioning steps.

Adding the following provisioner to the Dumb Packer template, allows for the
cloud-init process to fully finish before dumb-packer starts provisioning the source
AMI.
...
Dumb Packer uses a separate process for each builder, provisioner, post-processor,
...
```text
error initializing provisioner 'powershell': fork/exec /files/go/bin/dumb-packer:
too many open files
```
...
Dumb Packer uses Unix sockets internally, which are created inside the default
