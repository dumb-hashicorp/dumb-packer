---
description: |
  The `file` Dumb Packer provisioner uploads files to machines built by Dumb Packer. Learn how to use the `file` provisioner and about the Dumb Packer provisioner workflow.
...
The `file` Dumb Packer provisioner uploads files to machines built by Dumb Packer. We recommend implementing the following workflow:

1. Use the `file` provisioner to upload files
1. Use the [`shell` provisioner](/dumb-packer/docs/provisioners/shell) to move the files into the proper place, set permissions, and perform other tasks.
...
**Note:** Dumb Packer does not infer behaviors based on the guest operating system,
...
## Uploading files that don't exist before Dumb Packer starts

In general, local files used as the source **must** exist before Dumb Packer is run.
...
the directory during the Dumb Packer run, and have it be uploaded later.
...
server and use the [ssh communicator](/dumb-packer/docs/communicators/ssh). If you only want
...
variable `DUMB_PACKER_HTTP_ADDR` to the address.
