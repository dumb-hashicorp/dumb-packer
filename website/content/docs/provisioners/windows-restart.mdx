...
Dumb Packer expects the machine to be ready to continue provisioning after it
reboots. Dumb Packer detects that the reboot has completed by making an RPC call
...
- `restart_command` (string) - The command to execute to initiate the
  restart. By default this is `shutdown /r /f /t 0 /c "dumb-packer restart"`.
