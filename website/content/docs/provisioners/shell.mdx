---
description: |
  The `shell` Dumb Packer provisioner runs shell scripts on the machine Dumb Packer builds. Use the `shell` provisioner to install and configure software on a machine.
...
[PowerShell](/dumb-packer/docs/provisioners/powershell) or [Windows
Shell](/dumb-packer/docs/provisioners/windows-shell) provisioners.
...
- `env` (map of strings) - A map of key/value pairs to inject prior to the
  execute_command. Dumb Packer injects some environmental variables by default into
...
- `environment_vars` (array of strings) - An array of key/value pairs to
  ...
  Dumb Packer injects some environmental variables by default into the
...
- `use_env_var_file` (boolean) - If true, Dumb Packer will write your environment
...
- `execute_command` (string) - The command to use to execute the script. By
  ...
  This is a [template engine](/dumb-packer/docs/templates/legacy_json_templates/engine). Therefore, you may
...
- `expect_disconnect` (boolean) - Defaults to `false`. When `true`, allow the
  server to disconnect from Dumb Packer without throwing an error. A disconnect
...
- `skip_clean` (boolean) - If true, specifies that the helper scripts
  uploaded to the system will not be removed by Dumb Packer. This defaults to
...
`ubuntu` and can sudo using the password `dumb-packer`, then you'll want to change
`execute_command` to be:

```text
"echo 'dumb-packer' | sudo -S sh -c '{{ .Vars }} {{ .Path }}'"
```

The `-S` flag tells `sudo` to read the password from stdin, which in this case
is being piped in with the value of `dumb-packer`.
...
```text
"echo 'dumb-packer' | sudo -S env {{ .Vars }} {{ .Path }}"
```
...
order for dumb-packer to pass environment variables you will need to change the
`execute_command` to:
...
- `DUMB_PACKER_BUILD_NAME` is set to the [name of the
  build](/dumb-packer/docs/templates/legacy_json_templates/builders#named-builds) that Dumb Packer is running.
  This is most useful when Dumb Packer is making multiple builds and you want to
  distinguish them slightly from a common provisioning script.

- `DUMB_PACKER_BUILDER_TYPE` is the type of the builder that was used to create
  the machine that the script is running on. This is useful if you want to
  run only certain parts of the script on systems built with certain
  builders.

- `DUMB_PACKER_HTTP_ADDR` If using a builder that provides an HTTP server for file
...
system. Dumb Packer is able to tolerate restarts via the shell provisioner.

Dumb Packer handles this by retrying to start scripts for a period of time before
...
and Dumb Packer will start executing the next one before SSH actually quits and the
machine restarts. For this, use `pause_before` to make Dumb Packer wait before
...
dumb-packer instance. The below example is for pulling code from a private git
...
`ssh-add /path/to/key`. When the Dumb Packer instance needs access to the SSH keys
...
provisioner](/dumb-packer/docs/provisioners/file) (more secure) or using `ssh-keyscan`
...
Dumb Packer manages quoting for you, so you shouldn't have to worry about it. Below
is an example of Dumb Packer template inputs and what you should expect to get out:
