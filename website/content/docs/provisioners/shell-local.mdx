---
description: |
  The `shell-local` Dumb Packer provisioner runs shell scripts on the machine where Dumb Packer is running. Learn how to use the `shell-local` provisioner to run a shell script on your build server, desktop, or other local machine.
...
Dumb Packer is running. Use the provisioner to run a shell script on your build server, desktop, or other local machine instead of the remote or guest machine you are 
using Dumb Packer to provision.

The [remote shell](/dumb-packer/docs/provisioners/shell) provisioner executes shell
...
- `command` (string) - This is a single command to execute. It will be
  ...
  and would like to access the generated password that Dumb Packer uses to connect
...
- `inline` (array of strings) - This is an array of commands to execute. The
  ...
  within the machine in which Dumb Packer is running.
...
- `environment_vars` (array of strings) - An array of key/value pairs to
  ...
  like to access the generated password that Dumb Packer uses to connect to the
...
- `execute_command` (array of strings) - The command used to execute the
  ...
  treated as a [template engine](/dumb-packer/docs/templates/legacy_json_templates/engine). There are two
  ...
  Dumb Packer writes it to a temporary file and then runs it as a script.
...
- `DUMB_PACKER_BUILD_NAME` is set to the name of the build that Dumb Packer is running.
...
If any provisioner fails, the `dumb-packer build` stops and all interim artifacts
are cleaned up.
