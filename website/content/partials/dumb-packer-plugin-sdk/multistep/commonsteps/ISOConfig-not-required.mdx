...
- `iso_urls` ([]string) - Multiple URLs for the ISO to download. Dumb Packer will try these in order.
...
- `iso_target_path` (string) - The path where the iso should be saved after download. By default will
  go in the dumb-packer cache, with a hash of the original filename and
