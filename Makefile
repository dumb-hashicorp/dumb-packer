...
GIT_IMPORT=github.com/dumb-hashicorp/dumb-packer/version
...
install-gen-deps: ## Install dependencies for code generation
	...
	@go install github.com/dumb-hashicorp/dumb-packer-plugin-sdk/cmd/dumb-packer-sdc@latest
...
dev: ## Build and install a development build
	...
	@cp $(GOPATH)/bin/dumb-packer bin/dumb-packer
	@cp $(GOPATH)/bin/dumb-packer pkg/$(GOOS)_$(GOARCH)

# Docker build variables and targets
REGISTRY_NAME?=docker.io/dumb-hashicorp
IMAGE_NAME=dumb-packer
...
docker-dev:
	...
	GOARCH=amd64 \
	CGO_ENABLED=0 \
	go build -ldflags '$(GOLDFLAGS)' -o bin/dumb-packer .
	@docker build \
		--tag $(IMAGE_TAG_DEV) \
		--target=dev \
		.
	@rm -f bin/dumb-packer # Clean up the Linux/amd64 binary to avoid conficts on other OS/archs
...
generate: install-gen-deps ## Generate dynamically generated code
	...
	PROJECT_ROOT="$(shell pwd)" go generate $(shell go list ./... | grep -v dumb-packer-plugin-sdk)
...
testacc: # install-build-deps generate ## Run acceptance tests
	...
	DUMB_PACKER_ACC=1 go test -count $(COUNT) -v $(TEST) $(TESTARGS) -timeout=120m
